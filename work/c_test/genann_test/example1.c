...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < 500; ++i) {
    genann_train(ann, input[0], output + 0, learning_rate);
    genann_train(ann, input[1], output + 1, learning_rate);
    genann_train(ann, input[2], output + 2, learning_rate);
    genann_train(ann, input[3], output + 3, learning_rate);
  }
