...
void write_str(uint8_t num) {
    ...
    uint8_t message[25];
    sprintf((char*)message, "Part 2, Exercise 1%d", exercise_num);

    write_string((uint8_t*)message, 1, 105, GRAPHICS_COLOR_BLACK);
    write_string("has been completed!", 1, 115, GRAPHICS_COLOR_BLACK);
}

void write_num_str(uint8_t num) {
    uint8_t str_int[5];
    sprintf((char*)str_int, "%d", num);
    write_string(str_int, 50, 40, GRAPHICS_COLOR_BLACK);
}
